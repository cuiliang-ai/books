...
本章按**重要度递减**组织——先读完前两部分就能理解 SessionDB 在 Agent 中的核心角色，后面的内容是工程细节，可按需深入：

| 部分 | 节 | 内容 | 重要度 |
|------|-----|------|--------|
| **一、核心：存什么、怎么存** | §16.2–16.5 | 技术选型 → 数据模型 → 会话生命周期 → 消息读写 | ⭐⭐⭐ 必读 |
| **二、搜索：FTS5 全文检索** | §16.6–16.7 | 倒排索引原理 → 查询净化 | ⭐⭐ 理解 Session Search（第 19 章）的前置 |
| **三、工程护栏** | §16.8–16.11 | 连接配置 → 并发写入 → Schema 迁移 → 列表/导出 | ⭐ 遇到问题时再查 |

---

# 一、核心：存什么、怎么存

> 这部分回答三个基本问题：为什么用 SQLite？表结构长什么样？会话和消息如何创建、读取、更新？这是理解 SessionDB 的最短路径。
...
知道了"为什么用 SQLite"后，下一个问题是：**表里存了什么？** Hermes 的数据库由三部分组成：版本追踪表、两张核心业务表、一张 FTS5 虚拟表（FTS5 在 §16.6 单独讲解）。完整定义见 `SCHEMA_SQL`（`hermes_state.py:36-91`）。

### schema_version 表

只有一列 `version INTEGER NOT NULL`，当前值为 **6**。每次迁移递增，用于 §16.10 的递增式迁移判断。
...
知道了表结构后，下一个问题是：**会话怎么创建、结束、恢复？** 这是主循环（第 5 章）和上下文压缩（第 7 章）直接调用的核心 API。
...
会话创建后，主循环每轮都要做两件事：**写消息**和**读消息**。这是 SessionDB 被调用频率最高的操作。
...
> SessionDB 不只是被动存储——它还提供**主动搜索**能力。当 Agent 需要回忆"上次用户提到过什么数据库"时，不可能遍历所有会话逐条查看。FTS5 全文索引让这种搜索在毫秒级完成，是第 19 章 Session Search 工具的物理基石。
...
FTS5 有自己的查询语法——`"exact phrase"`、`AND/OR/NOT` 布尔运算、`prefix*` 前缀匹配。如果用户搜索 `hello AND`（悬挂运算符）或 `my-app.config.ts`（点号连字符会被分词），直接传给 `MATCH` 要么语法错误、要么语义错误。`_sanitize_fts5_query` 是一个六步净化器，确保任意用户输入都能安全执行：
...
> 以下内容不影响对 SessionDB 核心功能的理解，但它们解决了真实部署中的工程问题——多进程写入冲突、数据库版本升级、会话列表展示。如果你只是想理解 SessionDB 在 Agent 中的角色，可以跳过这部分；如果你要自己实现类似的持久化层，这些细节值得参考。

## 16.8 连接初始化与配置

`SessionDB.__init__()` 在每次 Hermes 启动时执行。它的初始化序列包含几个不显眼但重要的配置：
...
`state.db` 是一个长期存活的文件——用户可能用了几个月的 Hermes，里面存着几百个会话。但 Hermes 本身在持续迭代：v5 需要增加 11 个计费列来追踪 token 费用，v6 需要增加推理链列来支持新的推理模型。

问题来了：用户升级到新版 Hermes，启动时打开的还是那个老的 `state.db`，但新代码期望表里有新的列。如果直接访问不存在的列，程序会崩溃。

Schema 迁移解决的就是这个问题：**每次 `SessionDB.__init__()` 被调用时**（也就是每次 Hermes 启动时），它会检查数据库中的 `schema_version` 版本号，如果低于代码中的 `SCHEMA_VERSION = 6`，就自动执行缺失的 `ALTER TABLE ADD COLUMN` 操作，把老数据库的结构升级到新版本——整个过程对用户透明，不丢失任何数据。

### 实现方式

Hermes 采用最简洁的递增式方案。每个版本一个 `if current_version < N` 分支：
