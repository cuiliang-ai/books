...
| 阶段 | 职责 | 详见 |
|------|------|------|
| ① 入口仪式 | 防御性初始化：I/O 安全、模型恢复、输入清洗 | §5.2 |
| ② System Prompt | 一次构建，整会话复用；Gateway 从 SQLite 恢复 | §5.3 |
| ③ 入口预压缩 | 模型切换后上下文可能溢出，预先压缩到阈值内 | §5.4 |
| ④ 插件钩子 | 第三方上下文注入，不污染 System Prompt 缓存 | §5.5 |
...
回到 §5.1 的流程图，此时你已经走完了每一个方块的内部实现。六个阶段的要点回顾：

| 阶段 | 核心设计决策 |
|------|------------|
| ① 入口仪式 | 防御性重初始化 — 不信任调用者的环境状态 |
| ② System Prompt | 一次构建 + SQLite 恢复 — 最大化 prompt cache 命中 |
| ③ 入口预压缩 | 最多 3 轮 — 防止模型切换后的上下文溢出 |
| ④ 插件钩子 | 注入用户消息而非 System Prompt — 保护缓存前缀 |
| ⑤ 主循环 | 始终流式 + 协作式中断 + 恩典调用 |
| ⑥ 后处理 | 增量持久化 + 技能推送 — 为下一次会话做准备 |

这就是 Hermes Agent 的心跳——一个精心编排的循环，每次迭代都在"调用 LLM → 执行工具 → 注入结果"的闭环中推进任务。下一章将深入System Prompt的构建（阶段 ②），第 7 章将展开入口预压缩的内部算法（阶段 ③），第 9 章将解析错误恢复和降级的完整策略。
