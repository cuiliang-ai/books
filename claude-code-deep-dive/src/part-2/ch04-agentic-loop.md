...
| # | 条件 | 触发方式 | 处理方式 | 备注 |
|---|------|---------|---------|------|
| 1 | **end_turn** | 模型无 tool_use 输出 | 返回 `"completed"` | 正常完成 |
| 2 | **maxTurns** | turnCount > maxTurns | 返回 `"max_turns"` | 硬性上限 |
| 3 | **用户中断** | abort signal | 返回 `"aborted_streaming"` | 立即退出 |
| 4 | **max_tokens** | stop_reason = "max_tokens" | 注入恢复提示，继续循环 | 最多重试 3 次 |
| 5 | **API 错误** | image error / model error | 返回 `"image_error"` | 不可恢复 |
| 6 | **上下文阻塞** | token 超阻塞限制 | 返回 `"blocking_limit"` | 上层处理 |
| 7 | **Stop Hook** | Hook 返回 preventContinuation | 返回 `"stop_hook_prevented"` | Hook 决定 |
