...
回顾这本书走过的路径：从第 1 章的产品愿景出发，我们穿越了 `AIAgent` 的核心循环（第 3 章）、工具注册表的声明式架构（第 9 章）、六种终端后端的统一抽象（第 12 章）、Skill 闭环的自进化机制（第 14 章）、记忆系统的多层架构（第 15 章）、Gateway 的 15 平台消息分发（第 16 章）、上下文压缩的有损算法（第 7 章）、安全审批的多模式设计（第 20 章）、直到上一章的 RL 训练基础设施。
...
自进化在 Hermes 中有两个层次。第一个是**运行时**层次：Skills 系统（第 14 章）从成功经验中提炼可复用知识，Memory 系统（第 15 章）将用户画像和 agent 笔记持久化为 `§` 分隔的冻结快照，Session Search（`hermes_state.py`）通过 SQLite FTS5 实现跨会话召回。这三者形成闭环——解决问题 → 提炼 Skill → 持久记忆 → 下次会话召回 → 改进 Skill。
...
一个 `hermes model` 命令就能从 Claude 切到 GPT 再切到 DeepSeek。模型是可替换的基础设施，不是被锁定的平台。这个设计选择有代价——需要维护多个 adapter 和 credential 路径——但它让 Hermes 在任何模型生态中都能运行。
...
Gateway 模式最能体现这个原则。一个 Gateway 进程可以同时服务 Telegram、Discord、Slack、WhatsApp 等 15 个平台的用户，每个平台有自己的适配器（第 16 章），但 agent 核心逻辑是共享的。`BasePlatformAdapter` 的抽象（第 28 章）将平台差异封装在适配器层，上层代码只和 `MessageEvent` 打交道。
...
Hermes 的扩展模型是**注册表 + 抽象基类**，不是 fork-and-modify。`ToolRegistry` 是工具的注册表（第 9 章），`COMMAND_REGISTRY` 是命令的注册表（第 28 章），`MemoryProvider` 是记忆插件的抽象基类（第 15 章），`BaseEnvironment` 是终端后端的抽象基类（第 12 章），`BasePlatformAdapter` 是平台适配器的抽象基类（第 16 章），`ContextEngine` 是上下文引擎的抽象基类（第 7 章）。

每个扩展点都遵循相同的模式：定义契约 → 实现具体逻辑 → 注册。你不需要 fork 仓库就能添加新工具、新平台、新后端。这在 227K 行的单仓项目中尤其重要——如果每次扩展都需要修改核心代码，项目会迅速变得不可维护。

### 原则六：安全作为分层

安全不是一个单独的模块，而是贯穿多个层次的关注点。终端命令有 dangerous command 检测和审批系统（`tools/approval.py`，第 20 章）。记忆内容有 `sanitize_context()` 净化（防止 prompt injection，第 21 章）。URL 访问有 `is_safe_url()` SSRF 防护和 redirect guard（`gateway/platforms/base.py`）。系统提示有 `_scan_context_content()` 注入检测。Tirith 扫描提供了 pre-exec 安全检查。
...
**学习能力**。Claude Code 没有持久化的学习机制——每次会话从零开始，没有 Skill 系统，没有跨会话记忆，没有 session search。它依赖 `CLAUDE.md` 项目指令文件提供上下文，但这是手动维护的，不是自动积累的。Hermes 的 Skills + Memory + Session Search 闭环是根本性的架构差异——agent 会随着使用越来越好。
