...
前四章分别剖析了 Hermes Agent 的四大持久化子系统：SessionDB 负责结构化存储对话记录（第 16 章），Memory 系统将用户偏好和关键信息冻结到 MEMORY.md 与 USER.md（第 17 章），Skills 系统以渐进式披露管理可复用知识（第 18 章），Session Search 则通过 FTS5 + LLM 摘要实现跨会话检索（第 19 章）。这四个子系统彼此独立设计、独立演进，但它们共同构成了一个封闭的学习循环——Agent 不仅在单次会话中解决问题，还能从解决问题的过程中提取经验，持久化保存，并在未来的会话中重新激活。

这个循环可以用一条完整的数据流来描述。用户提出问题，Agent 在工具调用中摸索出解决方案。当对话积累到一定轮次后，系统会在后台悄然启动一个「影子 Agent」，它回顾整段对话，判断是否存在值得记住的用户偏好或可复用的操作模式。如果有，影子 Agent 通过 memory 工具将偏好写入 MEMORY.md，通过 skill_manage 工具将操作模式提炼成 SKILL.md 文件。下次用户开启新会话时，MEMORY.md 的内容会作为 system prompt 的一部分注入上下文，相关 Skill 会通过渐进式披露逐层展示。如果用户提到了以前做过的事情，session_search 会从 SQLite 的 FTS5 索引中找到历史对话并生成摘要。整个过程无需用户显式操作，甚至无需用户知道这些子系统的存在。
...
这个循环最精妙之处在于它完全是**被动触发**的。Agent 不会主动询问「我该记住什么？」——它只是在自然对话的间隙中观察、提炼、保存。用户感受到的是 Agent 越来越「懂」自己，但永远不会被学习过程打扰。
