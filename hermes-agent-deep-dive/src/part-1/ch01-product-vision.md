...
Aider 是一个 git-aware 的 AI 编程助手。它的核心优势是理解 git 仓库结构，能进行跨文件的一致性编辑。但它没有工具系统（不能运行命令、浏览网页、发消息），没有学习循环（不记住跨会话的经验），也不支持消息平台部署。

Hermes 的文件编辑能力通过 `tools/file_tools.py` 的 fuzzy patch 机制实现——它不追求 Aider 级别的 git 理解深度，但提供了**更广泛的工具生态**来弥补。

### Hermes vs Cursor

Cursor 是一个商业 IDE，AI 能力嵌入编辑器体验。它的优势是视觉化的 diff 预览、自然的编辑器集成、商业级的稳定性。但它是闭源商业产品，不能自部署，不能在消息平台上运行，不能在 Docker/SSH/Modal 上执行命令。

Hermes 走的是完全相反的路——它是一个**后端优先**的 Agent，终端和消息平台是它的自然栖息地，IDE 只是通过 ACP 适配器提供的辅助入口。

---

## 版本演进：从 v0.2 到 v0.8

Hermes Agent 在 7 个版本中从内部项目成长为完整的 Agent 平台。每个版本都有一个主题，理解这些主题就是理解架构演进的压力方向。

```
v0.2.0 (2026-03-12)  ━━  "The Foundation"
│  216 PR, 63 贡献者, 119 issues
│  基础能力到位：Gateway + MCP + Skills + ACP + 3,289 tests
│
v0.3.0 (2026-03-17)  ━━  "The Streaming & Plugins"
│  统一流式输出, 插件架构, 原生 Anthropic, Smart Approvals
│  Voice mode, 并发工具执行, PII 脱敏
│
v0.4.0 (2026-03-23)  ━━  "The Platform Expansion"
│  OpenAI 兼容 API server, 6 个新消息平台
│  4 个新推理提供商, MCP OAuth 管理, 上下文压缩重构
│
v0.5.0 (2026-03-28)  ━━  "The Hardening"
│  Nous Portal 400+ 模型, Hugging Face 集成
│  原生 Modal SDK, 供应链安全审计, Nix 打包
│
v0.6.0 (2026-03-30)  ━━  "The Multi-Instance"
│  Profiles 多实例隔离, MCP Server 模式, Docker 容器
│  Fallback Provider Chain, Feishu + WeCom 平台
│
v0.7.0 (2026-04-03)  ━━  "The Resilience"
│  可插拔 Memory Provider 接口, Credential Pool 轮换
│  Camofox 反检测浏览器, Gateway 竞态修复, 安全纵深
│
v0.8.0 (2026-04-08)  ━━  "The Intelligence"
│  209 PR, 82 issues
│  后台任务通知, Google AI Studio, MCP OAuth 2.1
│  GPT 工具调用自修复, 活动感知超时, 审批按钮
```

一个值得注意的模式：v0.2 到 v0.8 跨度只有 **27 天**（3月12日到4月8日）。这个速度说明了几件事：

1. 大量架构决策是在 v0.1 的内部开发期间就确定的
2. v0.2 带着 216 个 PR 和 63 个贡献者一起发布，说明开源那一刻已经有成熟的社区
3. 迭代速度体现了 Python 单仓的快速开发优势——没有编译步骤，没有 monorepo 工具链

另一个模式是**每个版本的安全加固都在增加**。v0.5 做了供应链审计（移除了被入侵的 `litellm`），v0.7 加了密钥泄露扫描，v0.8 加了 MCP 扩展的恶意软件扫描。这条线索说明 Hermes 的安全架构不是预先设计的，而是在真实部署中被现实**逼出来的**——正如所有生产系统的安全一样。
...
这种"概念延续、代码重写"的模式在开源项目中并不罕见（想想 Webpack 到 Vite，Express 到 Fastify），但它解释了为什么 Hermes 的架构在 v0.2 就已经如此成熟——它站在前人的肩膀上。
