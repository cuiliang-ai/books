---
title: "第 9 章：错误分类、限流与路由降级"
description: "自动 failover 如何工作"
sidebar:
  order: 9
---

import ErrorRoutingTree from '../../../components/diagrams/ErrorRoutingTree.tsx';

> **核心问题**：当 LLM API 返回错误时，Hermes 如何在毫秒级做出正确的恢复决策？是重试、换凭据、降级到备用模型，还是压缩上下文？

<ErrorRoutingTree client:only="react" />
