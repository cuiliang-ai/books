...
| 文件 | 行数 | 主要内容 | 详见 |
|------|------|---------|------|
| `run_agent.py` | ~10,600 | `AIAgent` 类、`run_conversation()` 主循环、`IterationBudget` | 第 3 章 |
| `model_tools.py` | ~580 | 工具编排层、`_discover_tools()`、`handle_function_call()`、同步-异步桥接 | 第 9、11 章 |
| `toolsets.py` | ~660 | Toolset 定义、`_HERMES_CORE_TOOLS`、`resolve_toolset()` 图展开 | 第 10 章 |
| `hermes_state.py` | — | `SessionDB`、SQLite + FTS5 全文检索 | 第 13 章 |
| `hermes_constants.py` | — | `get_hermes_home()`、`display_hermes_home()`、全局常量 | 第 26 章 |

### Agent 内部模块（`agent/`）

| 文件 | 主要内容 | 详见 |
|------|---------|------|
| `agent/prompt_builder.py` | 系统提示组装、`_scan_context_content()` 注入检测 | 第 8 章 |
| `agent/context_compressor.py` | 默认上下文压缩器、有损摘要 | 第 7 章 |
| `agent/context_engine.py` | `ContextEngine` 抽象基类 | 第 7 章 |
| `agent/prompt_caching.py` | Anthropic prompt caching 支持 | 第 8 章 |
| `agent/auxiliary_client.py` | 辅助 LLM 客户端（vision、summarization） | 第 6 章 |
| `agent/model_metadata.py` | 模型上下文长度、token 估算 | 第 6 章 |
| `agent/smart_model_routing.py` | `choose_cheap_model_route()`、双模型路由 | 第 6 章 |
| `agent/credential_pool.py` | 多凭据池、`least_used` 轮换策略 | 第 6 章 |
| `agent/anthropic_adapter.py` | Anthropic Messages → OpenAI 格式转换 | 第 6 章 |
| `agent/memory_manager.py` | `MemoryManager`、`sanitize_context()` 净化 | 第 15 章 |
| `agent/memory_provider.py` | `MemoryProvider` 抽象基类 | 第 15、28 章 |
| `agent/trajectory.py` | 轨迹保存、`convert_scratchpad_to_think()` | 第 29 章 |
| `agent/display.py` | `KawaiiSpinner`、tool preview 格式化 | 第 5 章 |
| `agent/skill_commands.py` | Skill slash 命令、skills 目录扫描 | 第 14 章 |
...
| `hermes_cli/commands.py` | `COMMAND_REGISTRY`、`CommandDef`、`SlashCommandCompleter` | 第 5、28 章 |
| `hermes_cli/callbacks.py` | 终端回调（clarify、sudo、approval） | 第 20 章 |
| `hermes_cli/setup.py` | 交互式 setup wizard | 第 2 章 |
| `hermes_cli/skin_engine.py` | 皮肤引擎、数据驱动 CLI 主题化 | 第 5 章 |
| `hermes_cli/auth.py` | Provider 凭据解析 | 第 6 章 |
| `hermes_cli/model_switch.py` | `/model` 切换流水线 | 第 6 章 |

### 工具模块（`tools/`）

| 文件 | 主要内容 | 详见 |
|------|---------|------|
| `tools/registry.py` | `ToolRegistry` 单例、`ToolEntry`、`tool_error()`/`tool_result()` | 第 9、28 章 |
| `tools/approval.py` | 危险命令检测 | 第 20 章 |
| `tools/terminal_tool.py` | 终端编排、后端选择 | 第 12 章 |
| `tools/process_registry.py` | 后台进程管理 | 第 12 章 |
| `tools/file_tools.py` | 文件读/写/搜索/patch | 第 11 章 |
| `tools/web_tools.py` | Web 搜索/提取（Parallel + Firecrawl） | 第 11 章 |
| `tools/browser_tool.py` | Browserbase 浏览器自动化 | 第 11 章 |
| `tools/code_execution_tool.py` | `execute_code` 沙箱 | 第 11 章 |
| `tools/delegate_tool.py` | 子 Agent 委派 | 第 11 章 |
| `tools/mcp_tool.py` | MCP 客户端（~1050 行） | 第 11 章 |
| `tools/skills_tool.py` | Skill CRUD 操作 | 第 14 章 |
| `tools/memory_tool.py` | Memory 读/写 | 第 15 章 |
| `tools/session_search_tool.py` | Session FTS5 搜索 | 第 13 章 |
...
| 文件 | 主要内容 | 详见 |
|------|---------|------|
| `gateway/run.py` | `GatewayRunner`、消息分发、slash 命令处理 | 第 16 章 |
| `gateway/session.py` | `SessionStore`、会话持久化 | 第 16 章 |
| `gateway/platforms/base.py` | `BasePlatformAdapter`、`MessageEvent`、消息生命周期 | 第 16、28 章 |
| `gateway/config.py` | `Platform` 枚举、`PlatformConfig` | 第 16 章 |
| `gateway/status.py` | 运行时状态、scoped lock | 第 26 章 |
